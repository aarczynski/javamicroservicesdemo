INSERT INTO company (name, geo_lat, geo_lon, created_at, updated_at)
VALUES ('TechCorp Poland sp. z o.o.', 52.2297, 21.0122, NOW(), NOW()),
       ('Santander Technology Kraków', 50.0647, 19.9450, NOW(), NOW()),
       ('CD Projekt S.A.', 51.1079, 17.0385, NOW(), NOW());

INSERT INTO skill (name, created_at, updated_at)
VALUES ('Java', NOW(), NOW()),
       ('Spring Boot', NOW(), NOW()),
       ('PostgreSQL', NOW(), NOW()),
       ('Python', NOW(), NOW()),
       ('React', NOW(), NOW());

INSERT INTO job_offer (company_id, title, description, salary_from, salary_to, currency, status, created_at, updated_at)
VALUES (
    (SELECT id FROM company WHERE name = 'TechCorp Poland sp. z o.o.'),
    'Senior Java Developer',
    'We are looking for an experienced Java developer to join our backend team.',
    18000.00, 24000.00, 'PLN', 'ACTIVE', NOW(), NOW()
),
(
    (SELECT id FROM company WHERE name = 'Santander Technology Kraków'),
    'Backend Engineer',
    'Join our fintech team building high-throughput payment systems.',
    16000.00, 22000.00, 'PLN', 'ACTIVE', NOW(), NOW()
),
(
    (SELECT id FROM company WHERE name = 'TechCorp Poland sp. z o.o.'),
    'Full Stack Developer',
    'Build modern web applications across the full stack.',
    15000.00, 20000.00, 'PLN', 'ACTIVE', NOW(), NOW()
),
(
    (SELECT id FROM company WHERE name = 'CD Projekt S.A.'),
    'Data Engineer',
    'Design and maintain data pipelines for game analytics.',
    14000.00, 19000.00, 'PLN', 'ACTIVE', NOW(), NOW()
),
(
    (SELECT id FROM company WHERE name = 'Santander Technology Kraków'),
    'Lead Java Developer',
    'Lead a team of engineers building our core banking platform.',
    22000.00, 28000.00, 'PLN', 'CLOSED', NOW(), NOW()
);

INSERT INTO job_offer_employment_type (job_offer_id, employment_type)
SELECT id, 'B2B'        FROM job_offer WHERE title = 'Senior Java Developer' UNION ALL
SELECT id, 'EMPLOYMENT' FROM job_offer WHERE title = 'Senior Java Developer' UNION ALL
SELECT id, 'EMPLOYMENT' FROM job_offer WHERE title = 'Backend Engineer'      UNION ALL
SELECT id, 'B2B'        FROM job_offer WHERE title = 'Full Stack Developer'  UNION ALL
SELECT id, 'B2B'        FROM job_offer WHERE title = 'Data Engineer'         UNION ALL
SELECT id, 'EMPLOYMENT' FROM job_offer WHERE title = 'Lead Java Developer';

INSERT INTO job_offer_skill (job_offer_id, skill_id, required_seniority_level, mandatory, weight, created_at, updated_at)
SELECT
    (SELECT id FROM job_offer WHERE title = 'Senior Java Developer'),
    (SELECT id FROM skill WHERE name = 'Java'),
    'SENIOR', true, 1.00, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Senior Java Developer'),
    (SELECT id FROM skill WHERE name = 'Spring Boot'),
    'MID', true, 0.80, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Backend Engineer'),
    (SELECT id FROM skill WHERE name = 'Java'),
    'MID', true, 1.00, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Backend Engineer'),
    (SELECT id FROM skill WHERE name = 'PostgreSQL'),
    'MID', false, 0.50, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Full Stack Developer'),
    (SELECT id FROM skill WHERE name = 'Java'),
    'MID', true, 0.80, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Full Stack Developer'),
    (SELECT id FROM skill WHERE name = 'React'),
    'MID', true, 0.80, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Data Engineer'),
    (SELECT id FROM skill WHERE name = 'Python'),
    'SENIOR', true, 1.00, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Data Engineer'),
    (SELECT id FROM skill WHERE name = 'PostgreSQL'),
    'SENIOR', true, 0.70, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Lead Java Developer'),
    (SELECT id FROM skill WHERE name = 'Java'),
    'LEAD', true, 1.00, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Lead Java Developer'),
    (SELECT id FROM skill WHERE name = 'Spring Boot'),
    'SENIOR', true, 0.90, NOW(), NOW()
UNION ALL
SELECT
    (SELECT id FROM job_offer WHERE title = 'Lead Java Developer'),
    (SELECT id FROM skill WHERE name = 'PostgreSQL'),
    'MID', false, 0.40, NOW(), NOW();
