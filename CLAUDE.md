...
| `platform-1`, `platform-2` | `role=platform:NoSchedule` | Gateway ingress (`.100` L2 announcement, restricted here since 2026-09-04 — see handoff), MetalLB controller, `local-path-provisioner`, self-hosted image registry (`.190`, replaces `ghcr.io` — see handoff), future Keycloak/SSO. Renamed from `worker-1`/`worker-2` and moved to `.2`/`.3` 2026-09-14 (kept at two nodes deliberately — decided against collapsing to one, see handoff) |
