...
| `worker-1`–`worker-5` | none (generic) | `app-candidates` (2-3 replicas, `HorizontalPodAutoscaler` on RPS since 2026-09-20 — see `k8s-cluster/manifests/candidates/hpa.yaml`), `app-job-offers` (2 replicas, manually scaled, no HPA). Since 2026-09-20, a hard `podAntiAffinity` guarantees exactly one app replica per worker (see `k8s-cluster/RPS-SCALING.md` — sharing a node measurably throttled the shared pod ~10x and caused a self-reinforcing Gateway connection imbalance); `worker-5` is the HPA's only headroom for a 3rd `app-candidates` replica, not general-purpose spare capacity — `maxReplicas: 3` is a hard ceiling until another worker exists. `worker-4` reclaimed from `db-3` 2026-09-20 after briefly losing it to `observability-3`; renumbered from `worker-3`–`worker-6` 2026-09-14 once `worker-1`/`worker-2` became `platform-1`/`platform-2`. `worker-5` added 2026-09-20, repurposed from `platform-2` |
