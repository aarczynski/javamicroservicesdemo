...
| `platform-1`, `platform-2` | `role=platform:NoSchedule` | Gateway ingress (`.100` L2 announcement, restricted here since 2026-09-04 — see handoff), MetalLB controller, `local-path-provisioner`, self-hosted image registry (`.190`, replaces `ghcr.io` — see handoff). Renamed from `worker-1`/`worker-2` and moved to `.2`/`.3` 2026-09-14. **2026-09-20: the "kept at two nodes deliberately" redundancy argument reversed** — MetalLB's L2 mode is active-passive per IP, so `platform-2` measured near-idle (~1.8% CPU) even at 1200rps while `platform-1` alone carried the Gateway traffic; `platform-2` is planned to move to a dedicated `sso-1` node for Keycloak instead (not yet executed — see handoff for the plan and the introspection-at-1500rps capacity risk to measure before integrating) |
| `worker-1`–`worker-4` | none (generic) | `app-candidates`, `app-job-offers` (2 replicas each as of 2026-09-11, manually scaled — no HPA yet). Since 2026-09-20, a hard `podAntiAffinity` guarantees exactly one app replica per worker (see `k8s-cluster/RPS-SCALING.md` — sharing a node measurably throttled the shared pod ~10x and caused a self-reinforcing Gateway connection imbalance), so 4 workers is now a real requirement for 4 total app pods, not just convenient. `worker-4` reclaimed from `db-3` 2026-09-20 after briefly losing it to `observability-3`. Renumbered from `worker-3`–`worker-6` 2026-09-14 once `worker-1`/`worker-2` became `platform-1`/`platform-2` |
