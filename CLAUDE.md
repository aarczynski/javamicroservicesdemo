...
| `observability-1`, `observability-2` | `role=observability:NoSchedule` | Prometheus, Grafana, Loki, otel-collector, Tempo components, Kafka, MinIO, Hubble Relay/UI (moved here 2026-09-14 — it's a dashboard, belongs with the rest of observability) |
...
| `worker-1`–`worker-4` | none (generic) | `app-candidates`, `app-job-offers` (2 replicas each as of 2026-09-11, manually scaled — no HPA yet), and future HPA replicas. Renumbered from `worker-3`–`worker-6` 2026-09-14 once `worker-1`/`worker-2` became `platform-1`/`platform-2` |
