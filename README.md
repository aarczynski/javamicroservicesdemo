...
* **load-background** - a module sending continuous 24/7 background load using k6. Runs as a Docker Compose service. Sends requests to `app-candidates` at 5–10 RPS following a sinusoidal curve with a 5-minute period. Starts only after the Spring Boot apps are healthy.
